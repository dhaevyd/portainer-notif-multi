...
def detect_changes(old_memory, new_containers):
    ...
    # Build name to old IDs mapping to detect restarts by name
    old_name_to_ids = {}
    for cid, data in old_memory.items():
        old_name_to_ids.setdefault(data['name'], set()).add(cid)

    # On first run, just report non-running containers
    if not old_memory:
        for ep_name, containers in new_containers.items():
            non_running = [
    ...
    for cid, container in new_flat.items():
        if cid not in old_memory:
            ...
            if old_ids_for_name:
                # Restarted container — use green tick ✅
                ...
                # Truly new container — use new emoji 🆕
                changes.setdefault(container['endpoint'], []).append(
    ...
    # Check removed containers, skip those that were marked as restarted
...
def main():
    ...
    while True:
        ...
        if containers:
            ...
            old_memory = new_memory  # update memory
        else:
